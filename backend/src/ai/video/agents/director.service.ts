...
import { buildReelsPlan, buildAiClipsPlan } from '../reels-plan-builder';
...
import { AiClipsAssetProvider } from '../assets/ai-clips-asset-provider';
...
export interface DraftAiClipsPlanParams {
  tenantId: string;
  appointmentId: string;
  clientId: string;
  technicianId: string;
...
export class DirectorService {
  ...
  // --------------------------------------------------------------------------
  // ai_clips (Phase 7): premium, opt-in, no technician images at all — every
  // scene asset is generated. Same critic → compliance → asset-resolution
  // shape as reels; the only new failure modes are the Asset agent's own
  // (AiClipsRateLimitError / AiClipsCostCeilingError), which land on the plan
  // as a normal failure, same as any other asset-resolution error.
  // --------------------------------------------------------------------------

  async draftAiClipsPlan(params: DraftAiClipsPlanParams): Promise<DraftSlideshowPlanResult> {
    if (params.sceneCount <= 0) {
      throw new DirectorError('At least one scene is required to draft an ai_clips plan');
    }

    const placeholderPlan = buildPlaceholderReelsPlan({
      ...params,
      imageUrls: [],
      brandTone: null,
      voiceoverEnabled: false,
    } as unknown as DraftReelsPlanParams, 'ai_clips');
    ...
    try {
      ...
      const assetProvider = params.assetProvider ?? new AiClipsAssetProvider();
      const assetResult = await assetProvider.resolveSceneAssets({
        tenantId: params.tenantId,
        sceneCopy,
        technicianImageUrls: [],
...
function buildPlaceholderReelsPlan(params: DraftReelsPlanParams, videoType: 'reels' | 'ai_clips' = 'reels'): VideoPlan {
  ...
  const scenes = Array.from({ length: params.sceneCount }, (_, index) => ({
    ...
    asset: { kind: videoType === 'ai_clips' ? 'generated_clip' : 'image', url: params.imageUrls[index] ?? null },
    motion: 'none',
