import type { VideoObjective } from '../contract';
...
export function buildScriptAgentDefinition(input: ScriptAgentInput): AgentDefinition<ScriptDraft> {
  return {
    ...
    systemPrompt: [
      'You write short-form 9:16 Instagram/TikTok slideshow copy for a beauty or wellness studio.',
...
export function buildScriptUserPrompt(input: ScriptAgentInput): string {
  ...
  return `Draft a ${input.sceneCount}-scene slideshow script.\n${notes}`;
}
